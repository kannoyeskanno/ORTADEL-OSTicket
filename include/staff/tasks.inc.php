...
// Make sure the cdata materialized view is available sample testing shean 
...
<?php csrf_token(); ?>
 ...
 <div class="table-responsive">
    <table class="table table-hover" border="0" cellspacing="1" cellpadding="2" width="100%">
        <thead class="bg-dark">
            <tr>
                ...
                foreach ($queue_columns as $k => $column) {
                    echo sprintf( 
                        '<th scope="col" style="max-width: %s; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"><a href="?sort=%s&dir=%s&%s" class="%s">%s</a></th>',
                        $column['width'],
                        $column['sort'] ?: $k,
                        $column['sort_dir'] ? 0 : 1,
                        $qstr,
                        isset($column['sort_dir']) ? ($column['sort_dir'] ? 'asc' : 'desc') : '',
                        $column['heading']
                    );
                }
                ?>
        ...
        <tbody>
            ...
            foreach ($tasks as $T) {
                ...
                ?>
                <tr class="table-active" id="<?php echo $T['id']; ?>">
                    <?php
                    ...
                    <td nowrap>
                        <a class="preview" href="tasks.php?id=<?php echo $T['id']; ?>"
                           data-preview="#tasks/<?php echo $T['id']; ?>/preview"><?php echo $number; ?></a>
                    </td>
                    <td nowrap>
                        <a class="preview" href="tickets.php?id=<?php echo $T['ticket__ticket_id']; ?>"
                           data-preview="#tickets/<?php echo $T['ticket__ticket_id']; ?>/preview"><?php echo $T['ticket__number']; ?></a>
                    </td>
                    <td nowrap><?php echo Format::datetime($T[$date_col ?: 'created']); ?></td>
                    <td>
                        <a <?php if ($flag) { ?> class="Icon <?php echo $flag; ?>Ticket" title="<?php echo ucfirst($flag); ?> Ticket" <?php } ?>
                            href="tasks.php?id=<?php echo $T['id']; ?>"><?php echo $title; ?></a>
                         <?php
                            if ($threadcount > 1)
                                echo "<small>($threadcount)</small>&nbsp;".'<i class="icon-fixed-width icon-comments-alt"></i>&nbsp;';
                            if ($T['collab_count'])
