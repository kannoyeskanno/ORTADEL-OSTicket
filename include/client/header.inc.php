            ...
            <ul class="nav pull-right">
                ...
                if ($nav && ($navs = $nav->getNavLinks()) && is_array($navs)) {
                  foreach ($navs as $name => $nav) {
                    echo sprintf('<li class="nav-item"><a class="%s %s nav-link link-dark px-2 mx-2" href="%s">%s</a></li>%s', $nav['active'] ? 'active' : '', $name, (ROOT_PATH . $nav['href']), $nav['desc'], "\n");
                  }
                } ?>
                <li class="nav-item">
...
<!-- 
              <span class="w-100 text-center text-black">
                <p class="h6">
        ...
        elseif ($nav) {
          if ($cfg->getClientRegistrationMode() == 'public') { ?>
                ...
                <div class="d-flex justify-content-center w-100 text-center text-black">
    <p class="h5">
        Welcome
        <?php
        if ($thisclient && is_object($thisclient) && $thisclient->isValid()
